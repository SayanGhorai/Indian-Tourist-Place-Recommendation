...
def prepare_places(
    ...
    output_csv: str = "data/places_genai_ready.csv",
    ...
    df["Date"] = pd.to_datetime(
        df.get("Date", pd.Series(dtype="datetime64[ns]")),
        errors="coerce"
    )
    ...
    df = df.drop_duplicates(subset=["City", "Place", "Review_clean"])

    # ---------------- Aggregate by Place ----------------
    place_agg = df.groupby(["City", "Place"], dropna=False).agg(
        review_text=("Review_clean", lambda x: " ".join(x.astype(str))),
        raw_reviews=("Review", lambda x: list(x)),
        avg_rating=("Rating", "mean"),
        review_count=("Review", "count")
    ).reset_index()

    # ---------------- Short Description ----------------
    place_agg["short_description"] = (
        place_agg["review_text"]
        .str.split()
        .apply(lambda x: " ".join(x[:30]))
    )
    ...
    def canonicalize_city(nc):
        if not nc:
            return ""
        return CITY_ALIAS_MAP.get(nc, nc)

    place_agg["City_canon"] = place_agg["City_norm"].apply(canonicalize_city)

    # ---------------- Quality Filtering ----------------
    place_agg = place_agg[
        (place_agg["review_count"] >= 2) &
        (place_agg["avg_rating"] >= 3)
    ...
    for tag, tag_query in TAG_QUERIES.items():
        ...
        tokens = set([
            t.strip()
            for t in re.split(r"\W+", tag_query.lower())
        ...
        for tok in tokens:
            mask = mask | place_agg["full_text"].str.contains(
